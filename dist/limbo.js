...
_manager2.default.getService('hello');
...
var ServiceManager = function () {
  ...
  _createClass(ServiceManager, [{
    ...
    value: function getService(name) {
      if (!window.applicationsServices || !window.applicationsServices[name]) {
        return null;
      }

      return window.applicationsServices[name];
    }
  }], [{
    key: 'addService',
    value: function addService(name, fn) {
      if (!window.applicationsServices) {
        window.applicationsServices = {};
      }
      if (window.applicationsServices[name]) {
        throw new Error('Key Already Exists!');
        return;
      }

      window.applicationsServices[name] = fn;
    }
  }]);
